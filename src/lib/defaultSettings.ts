/**
 * Per-game settings.json model (3Dos 1.5.1+ format), mirroring
 * server/lib/cartridge-settings.ts. Values are stored exactly as the console
 * writes them (snake_case keys, lowercase values); the *_LABELS maps give the
 * names the console shows in its menus.
 */
...
/** Display names for option values; values not listed are shown capitalized */
 ...
 * defaults vary by title and aren't published). Same as the server's
