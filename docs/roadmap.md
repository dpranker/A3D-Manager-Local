...
3. library.json editor: /Library/N64/Games/{Game Folder}/library.json, only for carts not in the built-in DB (console support since 3Dos 1.5.1, "Library customization for Unknown Cartridges"). Docs: https://www.analogue.co/developer/docs/platform/library-json. Schema: https://schemas.analogue.co/platform/3d/library.json (copy in docs/analogue-schemas/3d-library.json, fetched 2026-09-23). The schema is authoritative where the docs page disagrees.
   ...
   - Default retail colors for the All grid: data/retail-colors.json (41 NA colored-shell releases from https://retrorecall.net/guides/n64-colored-cartridges/, matched to USA cart IDs), lowest priority below the card and local copies. Regional variants and first-run-colored/later-gray reprints aren't modelled; the user is fine with covering most of them. A USA Turok 3 is listed black but the console recorded gray, so the console's table isn't identical to collector lists.
